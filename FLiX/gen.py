...
async def file_handler(client: Client, message: Message):
    ...
    if message.document:
        ...
        await message.reply_text("❌ ᴜɴsᴜᴘᴘᴏʀᴛᴇᴅ ғɪʟᴇ ᴛʏᴘᴇ", reply_to_message_id=message.id)
        return

    max_file_size = Config.get("max_telegram_size", 4294967296)
    if file_size > max_file_size:
        logger.warning("file too large | user=%s size=%s", user_id, file_size)
        await message.reply_text(
    ...
    processing_msg = await message.reply_text(
        "⏳ ᴘʀᴏᴄᴇssɪɴɢ ʏᴏᴜʀ ғɪʟᴇ...",
        reply_to_message_id=message.id,
    )

    try:
        forwarded = await message.copy(Config.DUMP_CHAT_ID)
        logger.info("file forwarded | user=%s msg_id=%s", user_id, forwarded.id)
    except Exception as exc:
        logger.error("failed to forward to dump channel | user=%s err=%s", user_id, exc)
        await processing_msg.edit_text(f"❌ ᴇʀʀᴏʀ ғᴏʀᴡᴀʀᴅɪɴɢ ᴛᴏ ᴄʜᴀɴɴᴇʟ: {exc}")
        return
    ...
    logger.info("file saved | user=%s hash=%s", user_id, file_hash)
    ...
    if is_streamable:
        buttons.append([
            InlineKeyboardButton(f"🎬 {small_caps('stream')}",   url=stream_url),
            InlineKeyboardButton(f"📥 {small_caps('download')}", url=download_url),
        ])
    else:
        buttons.append([
            InlineKeyboardButton(f"📥 {small_caps('download')}", url=download_url),
    ...
    text += f"\n\n💡 *{small_caps('tip')}:* ᴜsᴇ /revoke {secret_token} ᴛᴏ ᴅᴇʟᴇᴛᴇ ᴛʜɪs ғɪʟᴇ."
...
async def revoke_command(client: Client, message: Message):
    ...
    if not file_data:
        await message.reply_text(
            ...
            f"ᴛʜᴇ ғɪʟᴇ ᴅᴏᴇsɴ'ᴛ ᴇxɪsᴛ ᴏʀ ʜᴀs ʙᴇᴇɴ ᴅᴇʟᴇᴛᴇᴅ.",
            reply_to_message_id=message.id,
        )
        return

    if file_data["user_id"] != str(user_id) and user_id not in Config.OWNER_ID:
        logger.warning("revoke permission denied | user=%s token=%s", user_id, token)
        await message.reply_text(
    ...
    logger.info("file revoked | user=%s token=%s", user_id, token)
    await message.reply_text(
...
async def bandwidth_command(client: Client, message: Message):
    ...
    if user_id not in Config.OWNER_ID and not await db.is_sudo_user(str(user_id)):
        logger.warning("permission denied for /bandwidth | user=%s", user_id)
        await message.reply_text(
            f"❌ {small_caps('permission denied')}",
            reply_to_message_id=message.id,
        )
